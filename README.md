...
**下载安装包**：
- [var-gen-0.2.0-installer.zip](windows-installer/var-gen-0.2.0-installer.zip)

**一键安装**：
```bash
# 解压安装包
unzip var-gen-0.2.0-installer.zip
cd var-gen-0.2.0
...
- ✅ 支持中文路径和中文显示
- ✅ 自动处理依赖和环境配置
- ✅ 默认安装到：`C:\Users\{用户名}\AppData\Local\var-gen\`
...
- 🚀 **智能命名生成**：基于大语言模型（通义千问/讯飞星火）理解语义，生成贴切的英文变量名
...
- 📚 **双引擎架构**：
  - **大模型引擎**：基于通义千问(qwen-tiny)和讯飞星火(xinghuo-lite)
...
### 支持的大模型
目前支持以下大语言模型：
- **通义千问 (qwen-tiny)**: 阿里云DashScope平台，默认模型，响应快速
- **讯飞星火 (xinghuo-lite)**: 讯飞开放平台，轻量级版本
...
model = "qwen-tiny"  # 支持 "qwen-tiny" 或 "xinghuo-lite"
...
1. **大模型引擎**（在线模式）
   - **通义千问 (qwen-tiny)**: 阿里云DashScope平台，中文理解能力强，响应快速
   - **讯飞星火 (xinghuo-lite)**: 讯飞开放平台，语义理解准确，适合专业场景
   - 利用大语言模型深度理解语义，生成更智能、符合编程习惯的变量名
   - 需要有效API密钥和网络连接
...
### Q: 安装后在编辑器终端中无法使用？
**A:** 新版本(v0.2.1+)的安装脚本已经**完全解决**了这个问题！安装时会自动：

- ✅ 强制Windows重新加载环境变量
- ✅ 创建PowerShell配置文件，确保所有会话可用
- ✅ 生成CMD兼容的批处理包装器
- ✅ 进行全面的兼容性测试

**无需任何额外操作**，安装完成后立即在所有终端和编辑器环境中可用。

如果你使用的是旧版本，建议重新运行安装脚本：`powershell -ExecutionPolicy Bypass -File install.ps1`
